...
def myNetwork():
    ...
    h1.cmd('cd /home/ark/Desktop/Project')
    h1.cmd('sudo python -m http.server 80 --bind 10.0.0.1 &')
    h1.cmd('iperf -s -p 5050 &')
    h1.cmd('iperf -s -u -p 5051 &')
    sleep(2)
    for h in hosts:
        h.cmd('cd /home/ark/Desktop/downloads')
    for i in range(200):
        ...
        for j in range(10):
            ...
            if j <9:
                ...
                src.cmd("iperf -p 5050 -b 100M -t 5 -c 10.0.0.1 &")
                src.cmd("iperf -p 5051 -u -b 100M -t 5 -c 10.0.0.1 &")
            else:
                print("generating ICMP traffic between %s and h%s and TCP/UDP traffic between %s and h1" % (src,((dst.split('.'))[3]),src))
                src.cmd("ping {} -c 50 &".format(dst))
                src.cmd("iperf -p 5050 -b 100M -t 5 -c 10.0.0.1 &")
